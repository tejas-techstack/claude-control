...
2. Design before code: pick a palette (4-6 named hexes), a display+body type pairing (system or Google Fonts), a layout concept, and ONE signature element that makes this page memorable. Avoid the generic AI look — `references/design-guide.md` lists the exact defaults to avoid, plus color/type/contrast/a11y rules and a pre-ship checklist. Derive choices from the project subject itself.
3. Build a single self-contained `index.html` — start from `assets/starter.html` (theme-aware, accessible, zero external calls) and replace its placeholder palette/type/copy with your project-derived choices. No build step; responsive; keyboard focus visible; honors prefers-reduced-motion. Put it in `docs/` of the target repo (or `site/` if user prefers). Real project content, not lorem ipsum.
